...
A command line program that performs queries on data collected from people's electric vehicles in the form of a CSV.
...
## Description:
The application takes three arguments, the relative path to the data formatted as a CSV, a query and an argument for the given query.
...
- `average_daily_miles` function returns 0 for a car that is not in the dataset, but could potentially return a different output to differentiate between a car that did not drive and car that is not present

- I've decided to keep the program relatively simple since it is a very small application that handles realtively small amouts of data. It is intendedn as a primarily command line application that can be extended with additional functions (queries).
...
- processing time for `average_daily_miles` could be improved by caching the number of days total per dataset
...
* uses an odometer value from a given dataset as the basis for the calculation.
...
* the function returns a String "no car data available for selected date" in the case of date falling outside the dataset.
...
* when two or more data points in one day show a change in the odometer reading (the car was driven on that day)
...
One approach I would suggest would be to return number of cars that we have the most confidence did not drive on that day (and not count cases where we are not 100% sure) by checking the last odometer measurement of the day before and first odometer measurement of the day after. In case one of the two is missing (which would be the case for example the beginning/end of the dataset) we substitute the missing odometer measurement with either first of last measurement of the given day.
